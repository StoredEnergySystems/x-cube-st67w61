...
DEFAULT_CONFIG_FILE="mission_t02_flash_prog_cfg.ini"
CONFIG_FILE="mission_t02_flash_prog_cfg_auto.ini"
BINARY_PREFIX="st67w611m_mission_t02"
...
if [ "$BOARD_ID" = "NUCLEO-N657X0-Q" ]; then
    ...
    echo "Press any key to continue..."
    read -r junk
    STM32_Programmer_CLI -c port=swd mode=ur -w "$current_dir/NUCLEO-N657X0-Q_Binaries/ST67W6X_CLI_LWIP_FSBL.bin" 0x70000000 --extload "$CUBEPROGRAMMER/ExternalLoader/MX25UM51245G_STM32N6570-NUCLEO.stldr"
    if [ $? -ne 0 ]; then
      echo -e "${RED}!!!! Error detected !!!!${NC}"
      exit 1
    fi
